...
# URL del bot en Railway (cambiar por la tuya)
BOT_URL = st.secrets.get("BOT_URL", "https://crypto-bot-ia-dsk-production.up.railway.app/data")

# Sidebar fijo (no se actualiza en cada refresh)
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("Dashboard actualiza cada 10 segundos")

# Función para obtener datos (con caché de 10 segundos)
...
# Bucle de actualización automática usando st.rerun()
def refresh_data():
    while True:
        time.sleep(10)
        st.rerun()
...
# Auto-refresco cada 10 segundos usando JavaScript
st.markdown(
    """
